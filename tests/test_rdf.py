...
def test_get_variable_by_ontology_term(simple_ode_model, bad_annotation_model):
    """ Tests Model.get_variable_by_ontology_term() function. """
    # Test getting the time variable
    model = simple_ode_model
    var = model.get_variable_by_ontology_term((shared.OXMETA, 'time'))
    assert isinstance(var, sympy.Symbol)
    assert var.name == 'environment$time'

    # Test getting a non-existent variable
    with pytest.raises(KeyError):
        model.get_variable_by_ontology_term((shared.OXMETA, 'bert'))
    with pytest.raises(KeyError):
        model.get_variable_by_ontology_term(('http://example.com#', 'time'))
    ...
    with pytest.raises(ValueError, match='Multiple variables annotated with'):
        model.get_variable_by_ontology_term((shared.OXMETA, 'time'))

    # Annotation with a cmeta id that doesn't exist
    with pytest.raises(KeyError, match='No variable with cmeta id'):
        model.get_variable_by_ontology_term((shared.OXMETA, 'membrane_potential'))

    # Annotation of something that isn't a variable
    with pytest.raises(KeyError, match='No variable with cmeta id'):
        model.get_variable_by_ontology_term(
...
def test_get_ontology_terms_by_variable(bad_annotation_model):
    """ Tests Model.get_variable_by_ontology_term() function when the annotation is not correct. """
    # Test bad annotations
    model = bad_annotation_model
    ...
    for variable in model.graph:
        if str(variable) == '_c$v3':
            annotations = model.get_ontology_terms_by_variable(variable, shared.OXMETA)
            assert len(annotations) == 0


def test_get_ontology_terms_by_variable2(hh_model):
    """ Tests Model.get_variable_by_ontology_term() function when the annotation is not correct. """
    membrane_voltage_var = hh_model.get_variable_by_ontology_term((shared.OXMETA, "membrane_voltage"))
    annotation = hh_model.get_ontology_terms_by_variable(membrane_voltage_var, shared.OXMETA)
    assert len(annotation) == 1
    assert annotation[0] == "membrane_voltage"

    # Repeat test with wrong namespace
    annotation = hh_model.get_ontology_terms_by_variable(membrane_voltage_var, 'http://www.nottingam.ac.uk/')
    assert len(annotation) == 0

    # Repeat test without specifying namespace
    annotation = hh_model.get_ontology_terms_by_variable(membrane_voltage_var)
    assert len(annotation) == 1
    assert annotation[0] == "membrane_voltage"

    # Get a symbol without cmeta_id and check that get_ontology_terms_by_variable works
    symbol = [s for s in hh_model.get_state_variables() if not s.rdf_identity][0]
    annotation = hh_model.get_ontology_terms_by_variable(symbol)
    assert len(annotation) == 0
...
def test_has_ontology_term_by_variable(bad_annotation_model):
...
def test_get_rdf_annotation(simple_ode_model):
    ...
    params = model.get_variables_by_rdf((PYCMLMETA, 'modifiable-parameter'), 'yes')
...
def test_add_cmeta_id():
    ...
    v = model.get_variable_by_name('circle_x$x')
...
def test_transfer_cmeta_id():
    ...
    v1 = model.get_variable_by_ontology_term((shared.OXMETA, 'time'))
    v2 = model.get_variable_by_name('circle_x$x')
    ...
    v3 = model.get_variable_by_name('circle_x$y')
    assert v1.rdf_identity is None
    ...
    v4 = model.get_variable_by_ontology_term((shared.OXMETA, 'cytosolic_calcium_concentration'))
    assert v2.rdf_identity is not None
